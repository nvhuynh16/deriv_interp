...
// =============================================================================
// Primary reference:
//   scipy.interpolate.BSpline (conceptual interface)
//   https://github.com/scipy/scipy/blob/main/scipy/interpolate/_bsplines.py
//   https://docs.scipy.org/doc/scipy/reference/generated/scipy.interpolate.BSpline.html
//
// Implementation reference:
//   scipy.interpolate.BPoly (internal storage and algorithms)
//   Uses identical Bernstein basis storage and algorithms as BPoly.
//   See bpoly.h for detailed algorithm references.
//
...
//       (Conceptual - we store result in Bernstein form)
//
//   [3] B-spline to Bernstein conversion on each knot span
//       B-spline basis functions reduce to Bernstein polynomials on each interval
// =============================================================================

/**
 * B-Spline piecewise polynomial implementation
 *
 * Internally stores Bernstein coefficients for numerical stability.
 * Same basis and algorithms as BPoly (de Casteljau evaluation, [0,1] domain).
 *
 * Thread-safe: All methods are const after construction, no shared state.
 */
class BsPoly : public PolyBase<BsPoly> {
    friend class PolyBase<BsPoly>;

public:
    BsPoly(std::vector<std::vector<double>> coefficients,
           std::vector<double> breakpoints,
           ExtrapolateMode extrapolate = ExtrapolateMode::Extrapolate,
           int extrapolate_order_left = -1,
           int extrapolate_order_right = -1);

    static BsPoly from_array2d(ndarray::array2d<double> coefficients,
                               std::vector<double> breakpoints,
                               ExtrapolateMode extrapolate = ExtrapolateMode::Extrapolate,
                               int extrapolate_order_left = -1,
                               int extrapolate_order_right = -1);

    BsPoly(BsPoly&& other) noexcept = default;
    BsPoly(const BsPoly& other) = default;
    BsPoly& operator=(const BsPoly&) = delete;
    BsPoly& operator=(BsPoly&&) = delete;

    static BsPoly from_derivatives(std::vector<double> xi,
                                   std::vector<std::vector<double>> yi,
                                   std::vector<int> orders = {});

    static BsPoly from_power_basis(std::vector<std::vector<double>> power_coeffs,
                                   std::vector<double> breakpoints,
                                   ExtrapolateMode extrapolate = ExtrapolateMode::Extrapolate);
...
private:
    ...
    BsPoly make_poly(ndarray::array2d<double> coeffs, std::vector<double> breaks,
                     ExtrapolateMode extrap, int ol, int or_) const;
    BsPoly make_zero_poly() const;

    // =========================================================================
    // BsPoly-specific helpers (Bernstein basis)
    // =========================================================================
...
inline BsPoly::BsPoly(std::vector<std::vector<double>> coefficients,
                      std::vector<double> breakpoints,
                      ExtrapolateMode extrapolate,
                      int extrapolate_order_left,
                      int extrapolate_order_right)
...
inline BsPoly::BsPoly(FromArray2DTag,
                      ndarray::array2d<double> coefficients,
                      std::vector<double> breakpoints,
                      ExtrapolateMode extrapolate,
                      int extrapolate_order_left,
...
inline BsPoly BsPoly::from_array2d(ndarray::array2d<double> coefficients,
                                   std::vector<double> breakpoints,
                                   ExtrapolateMode extrapolate,
                                   int extrapolate_order_left,
                                   int extrapolate_order_right) {
    return BsPoly(FromArray2DTag{}, std::move(coefficients), std::move(breakpoints),
                  extrapolate, extrapolate_order_left, extrapolate_order_right);
}

// --- CRTP hooks ---

inline double BsPoly::evaluate_basis(int interval_idx, double t) const {
    return evaluate_bernstein(interval_idx, t);
}

inline double BsPoly::map_to_local(double x, double a, double b) const {
    return (x - a) / (b - a);
}

inline ndarray::array2d<double> BsPoly::compute_derivative_coefficients() const {
...
inline ndarray::array2d<double> BsPoly::compute_antiderivative_coefficients(double& running_integral) const {
    int new_degree = degree() + 1;
...
inline std::vector<double> BsPoly::elevate_degree_impl(const std::vector<double>& coeffs, int target_degree) {
    return elevate_degree(coeffs, target_degree);
}

inline BsPoly BsPoly::make_poly(ndarray::array2d<double> coeffs, std::vector<double> breaks,
                                ExtrapolateMode extrap, int ol, int or_) const {
    return BsPoly::from_array2d(std::move(coeffs), std::move(breaks), extrap, ol, or_);
}

inline BsPoly BsPoly::make_zero_poly() const {
    ndarray::array2d<double> zero_coeffs(1, num_intervals());
    for (int i = 0; i < num_intervals(); ++i) {
        zero_coeffs(0, i) = 0.0;
    }
    return BsPoly::from_array2d(std::move(zero_coeffs), breakpoints_, extrapolate_,
                                extrapolate_order_left_, extrapolate_order_right_);
}

// --- BsPoly-specific methods ---

inline double BsPoly::evaluate_bernstein(int interval_idx, double t) const {
    const int n = degree();
...
inline double BsPoly::binomial_coefficient(int n, int k) {
...
inline std::vector<double> BsPoly::hermite_interpolation(
    double x0, double x1,
    const std::vector<double>& y0,
    const std::vector<double>& y1) {
...
inline std::vector<double> BsPoly::elevate_degree(
    const std::vector<double>& coeffs,
    int target_degree) {
...
inline BsPoly BsPoly::from_derivatives(std::vector<double> xi,
                                       std::vector<std::vector<double>> yi,
                                       std::vector<int> orders) {
    ...
    return BsPoly(coefficients, xi);
}

inline BsPoly BsPoly::from_power_basis(std::vector<std::vector<double>> power_coeffs,
                                       std::vector<double> breakpoints,
                                       ExtrapolateMode extrapolate) {
    ...
    return BsPoly(bern_coeffs, breakpoints, extrapolate);
}

inline std::vector<std::vector<double>> BsPoly::to_power_basis() const {
    int n = degree();
